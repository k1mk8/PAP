...
public class MainActivity extends AppCompatActivity {
    ...
    public void showQR(View view){
        ...
        String QRData = LoginModule.StoredLogin;
        QRGenerator.setImageQR(QRData, QRBig);
    ...
    public void updateData(View view){
        String data = LoginModule.getUserData();
        try {
        ...
        login = separateData(jsData, "login");
        email = separateData(jsData, "email");
        name = separateData(jsData, "imie");
        surname = separateData(jsData, "nazwisko");
    }

    public String separateData(JSONObject jsdata, String data){
        try {
            return jsdata.getString(data);
        } catch (JSONException e) {
            e.printStackTrace();
            return "";
        }
    ...
    public void show_data_panel(View view){
        ...
        TextView change;
        if(isTrainer)
            change = findViewById(R.id.usernameDataViewTrainer);
        else
            change = findViewById(R.id.usernameDataView);
        change.setText(login);
        if(isTrainer)
            change = findViewById(R.id.emailDataViewTrainer);
        else
            change = findViewById(R.id.emailDataView);
        change.setText(email);
        if(!isTrainer) {
            change = findViewById(R.id.SubscriptionEndDataView);
            change.setText(name);
        }
        if(!isTrainer) {
            change = findViewById(R.id.subscriptionIDDataView);
            change.setText(surname);
        }
    }
    ...
    public void accept(View view) {
        TextView Paswword = findViewById(R.id.GiveOldPassword);
        String password = Paswword.getText().toString();
        Paswword = findViewById(R.id.GiveNewPassword);
        String newPassword = Paswword.getText().toString();
        Paswword = findViewById(R.id.GiveRepeatNewPassword);
        String repetedPassword = Paswword.getText().toString();
        if(!newPassword.equals(repetedPassword))
        {
            Toast.makeText(getApplicationContext(), "Given passwords do not match!", Toast.LENGTH_SHORT).show();
            return;
        }
        if(LoginModule.changePassword(login, password, newPassword))
            Toast.makeText(getApplicationContext(), "Password has been changed!", Toast.LENGTH_SHORT).show();
        else
            Toast.makeText(getApplicationContext(), "Password has NOT been changed!", Toast.LENGTH_SHORT).show();
        View Info_Layout = findViewById((R.id.Info_Layout));
        Info_Layout.setVisibility((View.VISIBLE));
        View Change_password_layout = findViewById(R.id.Change_password_layout);
